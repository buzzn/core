...
file=$1
schema=$2
file_base=$(echo $file | rev |cut -d'.' -f2- | rev)
adjusted=${file_base}_adjusted.sql
...
mysql -h $MYSQL_HOST -u $MYSQL_USER --password=$MYSQL_PASSWORD < ${adjusted}
