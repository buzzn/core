    ...
    Register.transaction do
      ...
      if @register.save!
        ...
        if !@register.virtual
          ...
          if @meter.registers.collect{|mp| mp.register_operator_contract}.any? && params[:register][:meter][:existing_meter] == t('add_existing_meter')
            @contract = @meter.registers.collect{|mp| mp.register_operator_contract}.first
