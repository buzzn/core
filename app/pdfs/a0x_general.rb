...
module Pdf
  class A0xGeneral < Generator
    ...
    def title
      "#{Buzzn::Utils::Chronos.now.strftime('%Y-%m-%d-%H-%M-%S')}-Auftragseingangsbestätigung-#{contract.localpool.base_slug}-#{contract.contract_number}-#{contract.contract_number_addition}-#{contract.contact.last_name}"
    end
    ...
    def build_tariff(tariff)
      {
        ...
        baseprice_euro_netto: german_div(tariff.baseprice_cents_per_month_before_taxes * 100),
        energyprice_cents_netto: german_div(tariff.energyprice_cents_per_kwh_before_taxes*100),
        energyprice_cents_brutto: german_div(tariff.energyprice_cents_per_kwh_before_taxes*100 * @vat.amount),
      }
    end
