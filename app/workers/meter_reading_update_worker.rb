      ...
      begin
        if request['status'] == "ok"
          if request['result'].any?
            request['result'].each do |reading|
              timestamp = DateTime.strptime(reading['time'].to_s,'%Q')
              watt_hour = reading['energy'] / 10000000.0 #energy is in 10^-10 kWh; convert to Wh
              Reading.create(register_id: register_id, timestamp: timestamp, watt_hour: watt_hour)
            end
            if init_reading
