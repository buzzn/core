  ...
  def perform(metering_point_id, resolution_format, containing_timestamp)
    if resolution_format == 'day'
      ...
      if @metering_point.group
        chart_data = @metering_point.group.chart(resolution_format, containing_timestamp)
        ...
        Score.create(mode: 'fitting', interval: interval_information[0], interval_beginning: interval_information[1], interval_end: interval_information[2], value: score_value, scoreable_type: 'MeteringPoint', scoreable_id: @metering_point.id)

        monthly_score = 0
        yearly_score = 0
        count_monthly_fittings = 0
        count_yearly_fittings = 0
        all_scores = @metering_point.scores.fittings.dayly
        ...
        Score.create(mode: 'fitting', interval: interval_information_month[0], interval_beginning: interval_information_month[1], interval_end: interval_information_month[2], value: monthly_score*1.0/count_monthly_fittings, scoreable_type: 'MeteringPoint', scoreable_id: @metering_point.id)

        interval_information_year = @metering_point.group.set_score_interval('year', containing_timestamp/1000)
        Score.create(mode: 'fitting', interval: interval_information_year[0], interval_beginning: interval_information_year[1], interval_end: interval_information_year[2], value: yearly_score*1.0/count_yearly_fittings, scoreable_type: 'MeteringPoint', scoreable_id: @metering_point.id)
      end
