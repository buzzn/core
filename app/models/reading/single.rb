...
module Reading
  class Single < ActiveRecord::Base
    ...
    scope :installed,     -> { with_reason(reasons[:device_setup], reasons[:device_change_2], reasons[:contract_change], reasons[:balancing_zone_change]) }
    scope :decomissioned, -> { with_reason(reasons[:device_removal], reasons[:device_change_1]) }
