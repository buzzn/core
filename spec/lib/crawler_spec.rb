...
describe Buzzn::Discovergy::Crawler do
  ...
  it 'parses single meter live response' do
    crawler = Buzzn::Discovergy::Crawler.new
    response = single_meter_live_response
    interval = Buzzn::Interval.live
    mode = 'in'
    two_way_meter = false
    external_id = broker.external_id
    result = crawler.parse_aggregated_data(response, interval, mode, two_way_meter, external_id)
  ...
  it 'parses virtual meter live response for each meter' do
    ...
    two_way_meter = false
    external_id = broker.external_id
    result = crawler.parse_collected_data(response, interval)
