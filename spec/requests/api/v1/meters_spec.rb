describe "Meters API" do
  ...
  [
    ...
    ['input', 'output'].each do |mode|
      it "does not get a #{mode} registers with #{token}" do
        meter = Fabricate(:meter)

        if token == :no_access_token
          get_without_token "/api/v1/meters/#{meter.id}/registers/#{mode}"
          expect(response).to have_http_status(401)
        else
          access_token = Fabricate(token)
          get_with_token  "/api/v1/meters/#{meter.id}/registers/#{mode}", access_token.token
  ...
  [:full_access_token, :smartmeter_access_token].each do |token|
    ...
    ['input', 'output'].each do |mode|
      it "gets related #{mode} register for Meter with #{token}" do
        access_token    = Fabricate(token)
