describe "billing-cycles" do
  ...
  it 'creates all regular billings' do
    ...
    POST "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{billing_cycle.id}/billings/regular", regular_token, accounting_year: 2016
    expect(response).to have_http_status(403)

    POST "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{billing_cycle.id}/billings/regular", manager_token, accounting_year: 2016
  ...
  it 'gets all billings' do
    GET "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{billing_cycle.id}/billings", regular_token
    expect(response).to have_http_status(403)

    GET "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{billing_cycle.id}/billings", manager_token
    expect(response).to have_http_status(200)
    expect(json).to match_array create_response
  end

  it 'updates a billing cycle' do
    PATCH "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{billing_cycle.id}", regular_token, name: 'abcd'
    expect(response).to have_http_status(403)

    PATCH "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{billing_cycle.id}", manager_token, name: 'abcd'
    expect(response).to have_http_status(200)
  ...
  it 'deletes a billing cycle' do
    ...
    DELETE "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{other_billing_cycle.id}", regular_token
    expect(response).to have_http_status(403)

    DELETE "/api/v1/groups/localpools/#{group.id}/billing-cycles/#{other_billing_cycle.id}", manager_token
