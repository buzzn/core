...
class Transactions::Admin::Exchange::GroupMemberDtvfExport < Transactions::Base
  ...
  def export_bank_account(target)
    column(target.bank_code)
    column(target.bank_name)
    column(target.bank_account_number)
    column(target.country_code)
    column(target.iban)
  ...
  def export_person(person)
    ...
    column("addresstyp") #TODO
    column("addresstyp") #TODO
    column("Bezeichnung")#TODO
