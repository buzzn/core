class Beekeeper::Importer::PowerTakerAnd3PartyContracts
  ...
  def run(localpool, powertaker_contracts, third_parties, registers, warnings)
    ...
    third_parties.each do |contract|
      begin
        ActiveRecord::Base.transaction do
          create_3_contract(localpool, contract, registers, warnings)
        end
      rescue => e
        logger.error("#{e} (meter buzznid: #{contract[:buzznid]})")
      end
  ...
  def create_3_contract(localpool, contract, registers, warnings)
