...
class Beekeeper::Import
  ...
  def import_localpool(record)
    ...
    Group::Localpool.transaction do
      ...
      Beekeeper::Importer::PowerTakerAnd3PartyContracts.new(logger).run(localpool, record.converted_attributes[:powertaker_contracts], record.converted_attributes[:third_party_contracts], registers, warnings)
