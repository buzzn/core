...
buzzn_team_names.each do |user_name|
  ...
  when 'christian'
    root_register = Fabricate(:register_60138988)
    root_register.contracts << Fabricate(:mpoc_christian, register: root_register)
    user.add_role :admin # christian is admin
  when 'philipp'
    root_register = Fabricate(:register_60009269)
    root_register.contracts << Fabricate(:mpoc_philipp, register: root_register)
  when 'stefan'
    @bhkw_stefan       = Fabricate(:bhkw_stefan)
    @forstenrieder_weg_register = root_register = Fabricate(:register_stefans_bhkw)
    root_register.contracts << Fabricate(:mpoc_stefan, register: root_register)
    root_register.devices << @bhkw_stefan
    user.add_role :manager, @bhkw_stefan
  when 'thomas'
    root_register = Fabricate(:register_ferraris_001_amperix)
    root_register.contracts << Fabricate(:mpoc_ferraris_0001_amperix, register: root_register)
    user.add_role :admin # thomas is admin
  when 'kristian'
    root_register = Fabricate(:register_kristian)
    root_register.contracts << Fabricate(:mpoc_buzzn_metering, register: root_register)
    user.add_role :admin # kristian is admin
  else
    root_register = Fabricate(:register)
  end
  if root_register
    user.add_role :manager, root_register
    user.add_role(:member, root_register)
  end
  root_register.contracts.each do |contract|
    user.contracting_parties.first.owned_contracts << contract
    contract.save
  end
...
karin.add_role :member, @forstenrieder_weg_register
...
group_hopf = Fabricate(:group_hopf, registers: [register_60118470])
