...
function bcChart(id, labels, data, title) {
  ...
  if (id == '#chart-two' || id == '#chart-three') {
    var colors = [
      '#fcba03',
      '#000',
      '#66a3ff',
  ...
  var maxvalue = data.reduce(function (a, b) {
    return Math.max(a, b);
  });
  var barLength = data.map(function (e) {
    return (e / maxvalue) * 100;
  });
  var barHeight = data.map(function (e) {
    return (e / sum) * $('#chart-two').height();
  });
  // calculate tick positions
  var ticks = [0, 1, 2, 3, 4, 5].map(function (e, i, arr) {
    return Math.round((maxvalue / (arr.length - 1)) * e);
  });
  // create child elements
  var barsHtml = !isHorizontal
    ? data.reduce(function (a, e) {
        return a + '<div class="bc-bar"></div>';
      }, '')
    : data.reduce(function (a, e) {
        return (
          a +
          '<div><div class="bc-label is-inline-block">label</div><div class="bc-bar is-inline-block"></div><div class="bc-value is-inline-block">value</div></div>'
        );
      }, '');
  barsHtml += data.reduce(function (a, e) {
    return a + '<div class="bc-tick"></div>';
  }, '<div class="bc-tick-container"><div class="bc-tick"></div>');
  barsHtml += '</div>';
  barsHtml += title ? '<h4>' + title + '</h4>' : '';
  var legendHtml = data.reduce(function (a, e, i) {
    return (
      a +
      '<p class="bc-legend"><span class="bc-legend-indicator"></span>' +
      labels[i] +
      ': ' +
      ((e / sum) * 100).toFixed(1) +
      ' %</p>'
  ...
  if (isHorizontal) {
    ...
    $(id + ' .bc-legend-indicator').each(function (i) {
